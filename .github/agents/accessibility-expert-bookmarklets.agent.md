...
model: GPT-4.1
tools: ['vscode', 'execute/testFailure', 'execute/getTerminalOutput', 'execute/runTask', 'execute/createAndRunTask', 'execute/runInTerminal', 'execute/runTests', 'read/problems', 'read/readFile', 'read/terminalSelection', 'read/terminalLastCommand', 'read/getTaskOutput', 'edit/editFiles', 'search', 'web', 'playwright/*']
---
...
- When a user asks to highlight all headings or evaluate them, use `mcp_playwright_browser_run_code` with the following Playwright code snippet:
```javascript
// Highlight all headings using Playwright's run_code tool
async (page) => {
  await page.addScriptTag({ url: 'https://cdn.jsdelivr.net/gh/pauljadam/bookmarklets@master/headings.js' });
  const iframes = await page.frames();
  for (const frame of iframes) {
    try {
      await frame.addScriptTag({ url: 'https://cdn.jsdelivr.net/gh/pauljadam/bookmarklets@master/headings.js' });
    } catch (e) {}
  }
}
```
